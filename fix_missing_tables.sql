-- =====================================================
-- 누락된 테이블 생성 및 Daily Quest 시스템 수정
-- =====================================================

-- 1. Daily Quest 질문 템플릿 테이블 (이미 있을 수도 있음)
...
CREATE INDEX IF NOT EXISTS idx_user_conversation_patterns_user ON user_conversation_patterns(user_id);
CREATE INDEX IF NOT EXISTS idx_user_learning_data_user ON user_learning_data(user_id, created_at DESC);
...
DROP POLICY IF EXISTS "Users can view their own patterns" ON user_conversation_patterns;
CREATE POLICY "Users can view their own patterns" ON user_conversation_patterns
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own patterns" ON user_conversation_patterns;
CREATE POLICY "Users can insert their own patterns" ON user_conversation_patterns
  FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own patterns" ON user_conversation_patterns;
CREATE POLICY "Users can update their own patterns" ON user_conversation_patterns
  FOR UPDATE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can view their own learning data" ON user_learning_data;
CREATE POLICY "Users can view their own learning data" ON user_learning_data
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own learning data" ON user_learning_data;
...
BEGIN
  ...
  today_quest_day := (days_since_start % 30) + 1; -- 30개 질문 순환
